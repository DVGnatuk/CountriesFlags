//
//  CountriesFlags.h
//  CountriesFlags
//

#ifndef CountriesFlags_h
#define CountriesFlags_h

#import <UIKit/UIKit.h>

typedef NS_ENUM(NSUInteger, CountriesFlags)  {
//        case Afghanistan
    CountriesFlagsAlandIslands,
//        case Albania
    CountriesFlagsAlgeria,
//        case AmericanSamoa
//        case Andorra
//        case Angola
//        case Anguilla
    CountriesFlagsAntiguaAndBarbuda,
//        case Argentina
    CountriesFlagsArmenia,
    CountriesFlagsAruba,
    CountriesFlagsAustralia,
    CountriesFlagsAustria,
    CountriesFlagsAzerbaijan,
    CountriesFlagsBahamas,
    CountriesFlagsBahrain,
    CountriesFlagsBangladesh,
//        case Barbados
    CountriesFlagsBelarus,
    CountriesFlagsBelgium,
//        case Belize
    CountriesFlagsBenin,
//        case Bermuda
//        case Bhutan
//        case Bolivia
    CountriesFlagsBosniaAndHerzegovina,
    CountriesFlagsBotswana,
//        case Brazil
//        case BritishIndianOceanTerritory
//        case BruneiDarussalam
    CountriesFlagsBulgaria,
    CountriesFlagsBurkinaFaso,
    CountriesFlagsBurundi,
    CountriesFlagsCaboVerde,
//        case Cambodia
//        case Canada
    CountriesFlagsCapeVerde,
//        case CaymanIslands
    CountriesFlagsCentralAfricanRepublic,
    CountriesFlagsChad,
    CountriesFlagsChile,
    CountriesFlagsChina,
//        case ChristmasIsland
//        case CocosIslands
    CountriesFlagsColombia,
    CountriesFlagsComoros,
    CountriesFlagsCookIslands,
    CountriesFlagsCostaRica,
    CountriesFlagsCoteDIvoire,
//        case Croatia
    CountriesFlagsCuba,
//        case Cyprus
    CountriesFlagsCzechRepublic,
    CountriesFlagsDemocraticRepublicCongo,
    CountriesFlagsDenmark,
    CountriesFlagsDjibouti,
//        case Dominica
//        case DominicanRepublic
//        case Ecuador
//        case Egypt
//        case ElSalvador
//        case EquatorialGuinea
//        case Eritrea
    CountriesFlagsEstonia,
//        case Eswatini
//        case FalklandIslands
    CountriesFlagsFaroeIslands,
//        case Fiji
    CountriesFlagsFinland,
    CountriesFlagsFrance,
    CountriesFlagsFrenchGuiana,
//        case FrenchPolynesia
    CountriesFlagsGabon,
    CountriesFlagsGambia,
//        case Georgia
    CountriesFlagsGermany,
    CountriesFlagsGhana,
//        case Gibraltar
    CountriesFlagsGreece,
    CountriesFlagsGreenland,
//        case Grenada
    CountriesFlagsGuadeloupe,
//        case Guam
//        case Guatemala
    CountriesFlagsGuinea,
    CountriesFlagsGuineaBissau,
    CountriesFlagsGuyana,
//        case Haiti
//        case VaticanCityState
    CountriesFlagsHonduras,
//        case HongKong
    CountriesFlagsHungary,
    CountriesFlagsIceland,
//        case India
    CountriesFlagsIndonesia,
//        case Iran
//        case Iraq
    CountriesFlagsIreland,
//        case IsleOfMan
    CountriesFlagsIsrael,
    CountriesFlagsItaly,
    CountriesFlagsJamaica,
    CountriesFlagsJapan,
//        case Jersey
    CountriesFlagsJordan,
//        case Kazakhstan
//        case Kenya
//        case Kiribati
//        case Kosovo
//        case Korea
    CountriesFlagsKuwait,
//        case Kyrgyzstan
    CountriesFlagsLaos,
    CountriesFlagsLatvia,
//        case Lebanon
//        case Lesotho
    CountriesFlagsLiberia,
    CountriesFlagsLibya,
//        case Liechtenstein
    CountriesFlagsLithuania,
    CountriesFlagsLuxembourg,
//        case Macao
    CountriesFlagsMadagascar,
//        case Malawi
    CountriesFlagsMalaysia,
    CountriesFlagsMaldives,
    CountriesFlagsMali,
//        case Malta
    CountriesFlagsMarshallIslands,
    CountriesFlagsMartinique,
    CountriesFlagsMauritania,
    CountriesFlagsMauritius,
//        case Mayotte
//        case Mexico
    CountriesFlagsMicronesia,
//        case Moldova
    CountriesFlagsMonaco,
//        case Mongolia
//        case Montenegro
//        case Montserrat
    CountriesFlagsMorocco,
//        case Mozambique
    CountriesFlagsMyanmar,
    CountriesFlagsNamibia,
    CountriesFlagsNauru,
//        case Nepal
    CountriesFlagsNetherlands,
    CountriesFlagsNetherlandsAntilles,
//        case NewCaledonia
    CountriesFlagsNewZealand,
//        case Nicaragua
    CountriesFlagsNiger,
    CountriesFlagsNigeria,
    CountriesFlagsNiue,
    CountriesFlagsNorthMacedonia,
//        case NorfolkIsland
//        case NorthernMarianaIslands
    CountriesFlagsNorway,
//        case Oman
    CountriesFlagsPakistan,
    CountriesFlagsPalau,
//        case PalestinianTerritory
    CountriesFlagsPanama,
//        case PapuaNewGuinea
//        case Paraguay
//        case Peru
//        case Philippines
//        case Pitcairn
    CountriesFlagsPoland,
//        case Portugal
    CountriesFlagsPuertoRico,
    CountriesFlagsQatar,
    CountriesFlagsRomania,
    CountriesFlagsRussianFederation,
    CountriesFlagsRwanda,
    CountriesFlagsRepublicCongo,
    CountriesFlagsReunion,
//        case SaintBarthelemy
//        case SaintHelena
    CountriesFlagsSaintKittsAndNevis,
    CountriesFlagsSaintLucia,
    CountriesFlagsSaintMartin,
//        case SaintPierreAndMiquelon
    CountriesFlagsSaintVincentAndTheGrenadines,
    CountriesFlagsSamoa,
//        case SanMarino
    CountriesFlagsSaoTomeAndPrincipe,
//        case SaudiArabia
    CountriesFlagsSenegal,
//        case Serbia
    CountriesFlagsSeychelles,
    CountriesFlagsSierraLeone,
    CountriesFlagsSingapore,
    CountriesFlagsSlovakia,
//        case Slovenia
    CountriesFlagsSolomonIslands,
    CountriesFlagsSomalia,
    CountriesFlagsSouthAfrica,
    CountriesFlagsSovietUnion,
//        case SouthGeorgiaAndSouthSandwichIslands
//        case Spain
//        case SriLanka
    CountriesFlagsSudan,
    CountriesFlagsSuriname,
//        case SvalbardAndJanMayen
//    CountriesFlagsSwaziland,
    CountriesFlagsSweden,
    CountriesFlagsSwitzerland,
    CountriesFlagsSyrianArabRepublic,
    CountriesFlagsTaiwan,
//        case Tajikistan
    CountriesFlagsTanzania,
    CountriesFlagsThailand,
    CountriesFlagsTimorLeste,
    CountriesFlagsTogo,
//        case Tokelau
    CountriesFlagsTonga,
    CountriesFlagsTrinidadAndTobago,
    CountriesFlagsTunisia,
    CountriesFlagsTurkey,
//        case Turkmenistan
//        case TurksAndCaicosIslands
    CountriesFlagsTuvalu,
//        case Uganda
    CountriesFlagsUkraine,
    CountriesFlagsUnitedArabEmirates,
    CountriesFlagsUnitedKingdom,
    CountriesFlagsUnitedStatesOfAmerica,
//        case Uruguay
    CountriesFlagsUzbekistan,
//        case Vanuatu
//        case Venezuela
    CountriesFlagsVietnam,
//        case VirginIslandsBritish
//        case VirginIslandsUnitedStates
    CountriesFlagsWallisAndFutuna,
    CountriesFlagsYemen
//        case Zambia
//        case Zimbabwe
};

#endif /* CountriesFlags_h */
